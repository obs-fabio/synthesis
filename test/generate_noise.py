"""Test Program for Labsonar Synthesis Library

This program generates synthetic noise based on a desired spectrum using the Labsonar Synthesis library.
It then analyzes the generated noise and compares it with the desired spectrum.
...
def main():
    ...
    for i in range(max_sample, len(desired_spectrum)):
        desired_spectrum[i] = max_db - (max_db - min_db) * math.sin(math.pi / 2 * ((i - max_sample) / (len(desired_spectrum) - max_sample)))
    ...
    scipy_wav.write(output_file, fs, sp_analysis.normalize(noise, 1))
